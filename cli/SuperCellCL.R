...
spec = matrix(c(
  ...
  'approx', 'x', 0, "logical", "Whether to use the do.approx option in SCimplify() to use approximate kNN in case of a large dataset",
  "nb_cells", "c", 1, "numeric", "Number of cells to subsample for an approximate approach, only considered in approx is TRUE"
), byrow=TRUE, ncol=5)
...
if (opt$cores > 1 & !is.null(opt$annotations)) {
  ...
  SCs <- foreach::foreach(sobj.label = SplitObject(sobj,split.by = opt$annotations)) %dopar% {
    ...
    do.approx <- opt$approx
    approx.N <- opt$nb_cells
